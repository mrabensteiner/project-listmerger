...
\uiname{origin list} to the \uiname{mergelist}. A merged item has a list
of its origin items (predecessors). They are either taken from the
mergelist (merged or moved), or directly taken from the origin list. The
...
The library itself is modularised into seven TypeScript files and
compiled with Gulp to a single \fname{.js} file. Minimised and Gzip
files, as well as a CSS stylesheet are added. ListMerger itself does not
define its own style, but certain CSS styling rules are highly
recommended and can be found in \fname{listmerger.css}. This stylesheet
contains the recommended minimum styling to build a proper example. The
only dependency at runtime is Mustache \parencite{mustache} to render
...
Section~\ref{sec:libu-init}. This function call is necessary to
initialise all functions of all files.
...
context of the user action. User actions are always tracked with the
\vname{history.log()} function, to make all actions completely
reproducible. Depending on the action, different parameters (like IDs,
arrays, or whole item objects) are stored. For this, the log function
has a function header with six optional parameters:
\begin{samepage}
\begin{lstlisting}[%
  language=TypeScript,
]
...
For a time, it was necessary to use JavaScript event listeners for tab
bars on a website. By adding click event listeners to the title, the
...
\elname{summary} element is needed as a child, which represents the
title. If no such summary element is present, the title is
\uiname{Details}. By default, a small triangle on the left indicates the
open/closed status of the content. This status is toggled by clicking on
the \elname{summary} element. The content does not necessarily have to
be an element and can also be plain text. To have a clear structure, it
is still advisable to have an element to give the content a better
structure, for example with a \elname{section} element. With the
\vname{open} attribute, the fold out of \elname{details} can be
controlled. The \vname{name} attribute can be used to only open one
\elname{details} box when having several instances with the same
\vname{name} attribute.
...
include:
\begin{itemize}
\item \liintro{\elname{container} as flexbox}: This ensures to have the
...
\item \liintro{Display details as content}: The section would be shown
  right next to the summary, but this display option shifts it to the
  end.

\item \liintro{Display open details pseudo-element as content}: This
  pseudo element shows only the content of the one \elname{details}
  element that is open.
...
solution is applied. A CSS class (\cssname{.compact} in the example) is
added to the tab list. The titles can not be simply hidden, as this
would give the observer an undetermined width and lever its
...
\includegraphics[frame,keepaspectratio,width=\linewidth,height=\halfh]
{images/merged-i.png}
\caption[Two Lists with a Merged Item]{%
Merged origin items (right top) appear greyed out and annotated with a
merge icon. The merged item itself (left) contains links to them. Items
can be dragged and dropped from the right to the left.
\imgcredit{Screenshot by Martin Rabensteiner.}
}
...
Items are created with a \elname{details} element. A class name and a
\cname{data-role} attribute are added from the predefined selectors. The
inner part is defined by a template when initialising. All values that
are defined for the item in the \vname{items} object can be used for
rendering. Some values are added depending on the current state. These
can be:
\begin{itemize}
...
The drag and drop functionality is based on the native HTML Drag and
Drop API \parencite{mdn-draganddrop} and implemented in the
\fname{events.ts} file. This enables dragging items from the origin
lists to the left to the mergelist on the right, as seen in
Figure~\ref{fig:mergeditems}. The listener for dragging is applied to
the app container. The listener for dropping is applied to the
mergelist only. Besides drag and drop, items can also be added using a
\uiname{Move} button per item, or a \uiname{Move Remaining} button per
list.

% Dragstart
On \vname{dragstart}, it is checked whether the item is eligible to be
dragged. If so, the item ID is stored in a local variable that can be
called in further functions. Thanks to the JavaScript \vname{closest()}
function, this works for inner elements too, for example when the image
inside is dragged, and not the item itself.

% Dragover
The \vname{dragover} listener is bound to the mergelist and handles
changes during the dragging state. An HTML class (\cssname{drop} by
default) is added to the list, or an item, depending on whereat the
dragged item is hovered. When there are already items in the mergelist,
a bar is shown at the position where the dragged item is approaching to
drop. This applies as well, when items within the mergelist are
rearranged.

% Drop
The mergelist also listens on \vname{drop}, which performs the further
actions depending on the target. When dropped to the list, the item is
cloned and appended to the mergelist at the indicated position. When
dropped over another item, the merge dialogue is called for editing the
values of the new item out of the merge. The merge is only performed
when confirmed by a button as described in Section~\ref{sec:merging},
otherwise nothing happens. The state of the origin item is adapted to
the action performed on drop.
...
The editing of items is not handled by the library directly, as this can
strongly depend on the individual implementation. Therefore, the
\vname{events.edit()} function can be called from the outside with the
parameters \vname{id}, \vname{key}, and \vname{value}. The ID defines
the item, whereas the key-value pair is a property of the item. The
history logging is only triggered when value is different. In the main
example, an inline editing functionality is implemented, as seen in
Figure~\ref{fig:edit}.
% Detaching
A detach button or link can be added to mergeitems and items that are
merged to loosen their connection. All attributes from the mergeitem
stay the same, except for the connection to the original item to detach.
This item then appears untouched in the origin list again. Two
attributes need to be set for the surrounding element in order to
execute detaching: \vname{data-from} contains the ID of the original
item, \vname{data-to} contains the ID of the mergeitem.
...
workarounds, and possible future tasks.
...
mergelist (merging cursor) or merged into another item (adding cursor).
...
\parencite{mdn-dropeffect}. While the related CSS \vname{cursor}
property offers a variety of 36 mouse cursors, in interaction with the
operating system \parencite{mdn-cursor}, this setting is not taken into
account as soon as a drag event is started. The \vname{dropEffect}
offers a selection of only four elements, all equivalent to options for
the CSS property (\vname{copy}, \vname{move}, \vname{link},
\vname{none}).
...
implementation, the event listener is bound to the mergelist and
therefore the same for the list and its items. Otherwise, the event
listener needs to be reassigned every time an item is changed. Google
Chrome does basically support the change during an event, but at some
points a \vname{none} cursor is wrongly shown.
...
lead to sufficient results. This browser mechanic is known and discussed
throughout several discussions and issue reports in the web
...
be made easier by browser vendors in future, and that this possible
changes can be taken into account for further improvements of this
library.



\subsection{Flexbox Layout within \vname{display: contents}}
...
tabs with the \elname{details} element, the CSS option \vname{display:
contents} is used once to let the title be in row and the inner content
only for one list. This option shows only the inner part of the content
and do not produce a box by themselves \parencite{mdn-display}.
...
value are mentioned in the documentation.

Different structures for HTML and CSS only tabs, like radio buttons and
labels \parencite{kliemann-radiotabs} were tried, but led to the same
problem. Eventually, a JavaScript \vname{ResizeObserver}
