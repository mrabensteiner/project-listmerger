...
The library itself is written in TypeScript and compiled with Gulp to a
single \fname{.js} file. Minimised and Gzip files, as well as a CSS
stylesheet are added. This stylesheet contains the recommended minimum
styling to build a proper example. The only dependency at runtime is
Mustache \parencite{mustache} to render user-specific templates.
...
\section{Styles}
ListMerger itself does not define any styles. These are entirely up
...
Another improvement option for accessibility is to make the inline editions (see Section ...) accessible, in the sense that they can also be used without mouseclicks
...
A dialog opens to compare the two items to merge, as seen in
...
{>{\leavevmode\kern-\tabcolsep}m{1.2cm}lY<{\kern-\tabcolsep}}
\toprule
\textbf{Icon} & \textbf{Filename} & \textbf{Description} \\
\midrule
\includegraphics[height=1cm]{icons/caret} & \fname{caret.svg} & Replaces
...
%
\includegraphics[height=1cm]{icons/check} & \fname{check.svg} &
Used to indicate that an item is added to the mergelist, but not merged. \\
%
\includegraphics[height=1cm]{icons/close} & \fname{close.svg} &
For close buttons in dialogs. \\
%
\includegraphics[height=1cm]{icons/detach} & \fname{detach.svg} &
To detach items from mergeitems or the other way around.\\
%
\includegraphics[height=1cm]{icons/dragicon} & \fname{dragicon.svg} &
Used to rearranged items in the mergelist. Not included in the example.\\
%
\includegraphics[height=1cm]{icons/edit} & \fname{edit.svg} &
Button for editing values. \\
%
\includegraphics[height=1cm]{icons/merge} & \fname{merge.svg} &
Button to merge and also as indicator for items that are merged into another. \\
%
\includegraphics[height=1cm]{icons/move} & \fname{move.svg} &
For moving an item to the mergelist or back. \\
%
\includegraphics[height=1cm]{icons/moveall} & \fname{moveall.svg} &
Move all remaining items to the mergelist. \\
%
\includegraphics[height=1cm]{icons/openfile} & \fname{openfile.svg} &
Open a file as source for lists and items.\\
%
\includegraphics[height=1cm]{icons/redo} & \fname{redo.svg} &
History redo.\\
%
\includegraphics[height=1cm]{icons/save} & \fname{save.svg} & 
Save the current state of all lists. \\
%
\includegraphics[height=1cm]{icons/save_orange} & \fname{save_orange.svg} &
Save the current state of an item when editing.\\
%
\includegraphics[height=1cm]{icons/undo} & \fname{undo.svg} &
History undo.\\
\bottomrule
\end{tabularx}
\end{footnotesize}
...
mergelist (moving cursor) or merged into another item (adding cursor). The
\fname{event.datatransfer.dropEffect} property in JavaScript defines the
visual behaviour of the mouse cursor during a drag event
