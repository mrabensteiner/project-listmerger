...
context of the merge keyword. User actions are always tracked with the
...
has a function header with 6 optional parameters:
\begin{samepage}
\begin{lstlisting}[%
  language=TypeScript,
]
...
\uiname{Details}. By default, a small triangle on the left indicates
...
\cname{data-role} attribute are added from the predfined selectors. The
...
\fname{events.ts} file. The listener for dragging is applied to the app
container. The listener for dropping is applied to the mergedlist only.
Besides drag and drop, items can also be added using a \uiname{Move}
button per item, or a \uiname{Move Remaining} button per list.
...
history logging is only triggered when value is different.
% Detaching
